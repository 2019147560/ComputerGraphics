...
export class SquarePyramid {
    constructor(gl, options = {}) {
        ...
        // Map the 4 side faces into consecutive horizontal regions of the texture
        // Each side uses 1/4 of the texture width. For face i (0..3): u0 = i*0.25, u1 = (i+1)*0.25
        // We map the base edge of each triangular face to (u0,0)-(u1,0) and the apex to the middle at v=1.
        this.texCoords = new Float32Array([
            // front face (v1, v0, v_apex)  -> region 0 (u in [0.00,0.25])
            0.00, 0.0,   0.25, 0.0,   0.125, 1.0,
            // right face (v0, v3, v_apex)  -> region 1 (u in [0.25,0.5])
            0.25, 0.0,   0.50, 0.0,   0.375, 1.0,
            // back face (v3, v2, v_apex)   -> region 2 (u in [0.5,0.75])
            0.50, 0.0,   0.75, 0.0,   0.625, 1.0,
            // left face (v2, v1, v_apex)   -> region 3 (u in [0.75,1.0])
            0.75, 0.0,   1.00, 0.0,   0.875, 1.0,
            // bottom face (v2, v3, v0, v1) - keep full texture mapping for bottom
            0, 0, 1, 0, 1, 1, 0, 1,
        ]);
